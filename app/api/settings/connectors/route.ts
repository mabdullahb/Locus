...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (type === "webhook") {
      ...
      // The actual send path (lib/export/connectors/webhook.ts) already
      // rejects a blocked URL, but only at delivery time. Saving one anyway
      // showed as "Configured" with no indication it would never actually
      // deliver until the user clicked Test or ran a real export. Reject it
      // here too, at the point the user can still act on the feedback.
      try {
        await assertOutboundUrlAllowed(webhookUrl);
      } catch (err) {
        return NextResponse.json(
          { error: err instanceof Error ? err.message : "Webhook URL not allowed" },
          { status: 400 },
        );
      }
