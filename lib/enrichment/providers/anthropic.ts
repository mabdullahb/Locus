...
export const anthropicProvider: EnrichmentProvider = {
  ...
  async enrich(
    ...
    // (`{ type: "error", error: { message } }`) doesn't have `.content`, so
    // this silently fell through to an empty string, which parseJsonResponse
    // then reports as a clean "no email found" instead of a real failure.
    // Confirmed live: an invalid key, rate limit, or bad model name all
    // produced a "not found" result with zero error anywhere.
    if (!response.ok) {
