...
export class WebhookConnector extends BaseConnector {
  ...
  async send(data: ExportPayload, config: Record<string, string>): Promise<ConnectorResult> {
    ...
    try {
      await assertOutboundUrlAllowed(config.webhookUrl);
    } catch (err) {
      return {
        success: false,
    ...
    try {
      ...
      const response = await fetch(config.webhookUrl, {
        ...
        // A 3xx to an internal target would otherwise sidestep the SSRF check
        // above. A webhook receiver has no reason to redirect.
        redirect: "error",
